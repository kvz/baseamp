...
(function() {
  ...
  Api = (function() {
    ...
    Api.prototype._uploadItems = function(type, displayField, items, remoteIds, cb) {
      ...
      q = async.queue((function(_this) {
        return function(item, qCb) {
          ...
          if (method === "put" && !_this._itemDiffs(remoteItem, displayField, payload)) {
            debug("SKIP " + (_this._human(payload, displayField)));
            return qCb();
          }
    ...
    Api.prototype._human = function(item, displayField) {
    ...
    Api.prototype._itemDiffs = function(remoteItem, displayField, payload) {
      ...
      for (key in payload) {
        ...
        if (payload[key] !== remoteItem[key]) {
          debug("CHANGE '" + (this._human(payload, displayField)) + "'. Payload's key '" + key + "' is '" + val + "' while remoteItem's is '" + remoteItem[key] + "'");
          return true;
        }
      }
      return false;
