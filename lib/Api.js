...
(function() {
  ...
  Api = (function() {
    ...
    Api.prototype._assigneeToCallsign = function(assignee, remoteIds) {
      if (assignee == null) {
        return void 0;
      }
      if (assignee.callsign != null) {
    ...
    Api.prototype._personIdMatch = function(callsign, remoteIds) {
      var assignee, id, person, _ref;
    ...
    Api.prototype._uploadItems = function(type, displayField, items, remoteIds, cb) {
      ...
      q = async.queue((function(_this) {
        return function(item, qCb) {
          var method, opts, payload, remoteItem;
          ...
          if (payload.assignee != null) {
            payload.assignee = _this._personIdMatch(payload.assignee, remoteIds);
          }
          if (method === "put" && !_this._itemDiffs(type, remoteItem, displayField, payload, remoteIds)) {
            return qCb();
          }
          if (!payload[displayField]) {
            debug("SKIP empty weird todo with id: " + (payload != null ? payload.id : void 0));
            return qCb();
          }
    ...
    Api.prototype._itemDiffs = function(type, remoteItem, displayField, payload, remoteIds) {
      var diff, key, remVal, val;
      diff = [];
      for (key in payload) {
        val = payload[key];
        remVal = remoteItem[key];
        if (key === "assignee") {
          val = this._assigneeToCallsign(val, remoteIds);
          remVal = this._assigneeToCallsign(remVal, remoteIds);
        }
        if (key === "completed" && val === false && remVal && true) {
          continue;
        }
        if (val !== remVal) {
          debug("CHANGE " + (this._human(type, payload, displayField)) + ". Payload's '" + key + "' is '" + val + "' while remoteItem's is '" + remVal + "'");
          return true;
        }
      }
      return false;
    ...
    Api.prototype.uploadTodoLists = function(localLists, cb) {
      ...
      return async.waterfall([
        (function(_this) {
          ...
          return function(callback) {
            return _this._uploadItems("lists", "name", localLists.lists, remoteIds, function(err, pushed) {
          ...
          return function(callback) {
            ...
            return _this._uploadItems("todos", "content", allTodos, remoteIds, function(err, pushed) {
      ...
      return this._request(this.endpoints["get_people"], null, (function(_this) {
        return function(err, people) {
          ...
          for (i = _i = 0, _len = people.length; _i < _len; i = ++_i) {
            ...
            if (__indexOf.call(callsigns, callsign) >= 0) {
