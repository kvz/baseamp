...
(function() {
  ...
  Api = (function() {
    ...
    Api.prototype._uploadItems = function(type, displayField, items, remoteIds, cb) {
      var errors, positionChangesSaved, q;
      errors = [];
      positionChangesSaved = 0;
      q = async.queue((function(_this) {
        return function(item, qCb) {
          var delta, field, method, opts, others, payload, remoteItem;
          ...
          if (method === "put") {
            delta = _this._itemDiffs(type, remoteItem, displayField, payload);
            if (delta.length === 0) {
              debug("SKIP " + (_this._human(type, payload, displayField)));
              return qCb();
            } else if (__indexOf.call(delta, "position") >= 0) {
              ...
              if (positionChangesSaved > 1) {
                ...
                if (others.length === 0) {
                  debug("SKIPOS " + (_this._human(type, payload, displayField)));
                  return qCb();
                } else {
